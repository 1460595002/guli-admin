        ...
        <el-form label-width="120px">
            ...
            <el-form-item label="选择Excel">
                <el-upload ref="upload" :auto-upload="false" :on-success="fileUploadSuccess" :on-error="fileUploadError"
                    :disabled="importBtnDisabled" :limit="1" :action="BASE_API+'/serviceedu/subject/addSubject'" name="file"
...
<script>  export default {
        ...
        methods: {
            submitUpload() {
                ...
                this.loading = true
                //js:document.getElmentById("upload").submit()
                this.$refs.upload.submit()
            },
            //导入成功回调函数
            fileUploadSuccess(response) {
                if (response.success === true) {
                    this.fileUploadBtnText = '导入课程成功'
                    this.loading = false
                    this.$message({
